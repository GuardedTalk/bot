...
import (
	...
	"github.com/GuardedTalk/bot/util"
...
func (o *OpusEncoder) Encode(pcm []float32, inputChannelCount, inputSampleRate int) ([]OpusFrame, error) {
	...
	if inputSampleRate != opusSampleRate {
		pcm = util.Resample(pcm, inputSampleRate, opusSampleRate)
	}
	...
	for _, frame := range frames {
		...
		if err != nil {
			fmt.Println("error encoding opus frame")
...
func (o *OpusEncoder) chunkPcm(pcm []float32, sampleRate int) []PcmFrame {
	...
	for idx <= totalFrames {
		...
		if pcmLen > outputFrameSize {
			...
			fmt.Printf("a partial frame len %d padding with %d silence samples \n", pcmLen, len(silence))
